"""Kiểm SummarizeAgent — đủ 6 bước, cưỡng chế manifest (doc 19 P-M0-5, lát 5b).

test_summarize_full_lifecycle_all_6_steps tích hợp THẬT: Registry (4a) +
StubAdapter (4b) + StateStore/EventBus (lát 1-2) + AgentContext (5a) — không
mock — chứng minh các lát cắt trước thật sự ráp được với nhau.
"""
...
_PROMPTS_DIR = _REPO_ROOT / "agents" / "summarize" / "prompts"

_TEST_MANIFEST = AgentManifest(
    agent_id="summarize.agent",
    version=1,
    needs=["text"],
    produces=["summary"],
...
def test_manifest_loaded_from_yaml() -> None:
    assert SummarizeAgent.manifest.agent_id == "summarize.agent"
    assert "text.generate@1" in SummarizeAgent.manifest.capabilities
    assert "summary.created" in SummarizeAgent.manifest.emits
...
async def test_review_rejects_empty_summary() -> None:
    agent = SummarizeAgent()
    result = await agent.review(ExecResult(data={"summary": ""}))
...
async def test_summarize_full_lifecycle_all_6_steps(store: StateStore, tmp_path: Path) -> None:
    ...
    async def call_capability(
        ...
        assert any(p.provider_id == "stub.deterministic" for p in providers), (
            "Registry phải tìm thấy provider stub qua provider.yaml thật"
        )
    ...
    events.subscribe("test", "summary.created", _handler)

    async def emit_event(event_type: str, payload: dict) -> None:
        await events.publish(event_type, source="agent.summarize", payload=payload)
    ...
    ctx = AgentContext(
        ...
        agent_id="summarize.agent",
        prompts_dir=_PROMPTS_DIR,
        manifest=SummarizeAgent.manifest,
    ...
    inputs = {"text": "PAOS là một hệ điều hành AI cá nhân chạy local-first."}
    validation = await agent.validate(inputs)
    assert validation.ok is True

    plan = await agent.think(inputs)
    assert "PAOS là một hệ điều hành" in plan.prompt

    exec_result = await agent.execute(plan)
    assert "[stub:text.generate]" in exec_result.data["summary"]
    ...
    assert (workspace / artifacts[0].path).read_text(encoding="utf-8") == exec_result.data[
        "summary"
    ]
