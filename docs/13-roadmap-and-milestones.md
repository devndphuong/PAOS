...
- [ ] KG có ≥ 100 node từ sử dụng thật, mọi edge có provenance — **một phần** (P-M5-3): cơ chế XÂY XONG và CHẠY THẬT — `KnowledgeStore`/`KnowledgeExtractor` (`apps/paosd/knowledge_store.py`, `apps/paosd/knowledge_extractor.py`) trích entity từ artifact thật (`plan.created`/`script.created`/`summary.created`), mọi node có `confidence`/`first_seen`/`last_seen`, MỌI edge có `provenance_json` (event + artifact_id + process_id, đủ để P-M5-4 lọc theo privacy sau này). Số **≥ 100 node từ sử dụng thật** CHƯA đạt và KHÔNG thể đạt trong 1 lát cắt implementation — cần người dùng dùng PAOS thật qua thời gian (đúng tinh thần P-M5-1/P-M5-2 đã ghi cho 2 exit criteria phía trên), không phải thứ một phiên code có thể tạo ra trung thực (chạy giả 100 job chỉ để đạt số đếm là "giả vờ làm", vi phạm P8).
- [x] Memory L3 không bao giờ rời máy khi `privacy: private` (test đối kháng) — **đạt (P-M5-4)**: `apps/paosd/router.py::Router.call(..., contains_private_l3=True)` chặn VÔ ĐIỀU KIỆN mọi candidate `provider_class == "cloud"`, dựa vào CLASS CẤU TRÚC (không tin self-declaration `privacy:` của chính provider — chống provider khai gian/cấu hình sai). Test đối kháng THẬT ở `tests/apps/paosd/test_privacy_filter.py` (7 đường tấn công: provider cloud khai gian `privacy: private`; đếm `invoke()` KHÔNG BAO GIỜ được gọi; event `privacy.cloud_send.blocked` không rò rỉ payload; Decision Record không rò rỉ payload; thông điệp lỗi không rò rỉ payload; cache không bị nhiễm từ lượt bị chặn; mặc định `False` thật sự opt-in). `paosctl memory forget` (xóa cứng thật, ADR-0029) + `export`/`import` JSON hoàn tất phần còn lại của doc 07 §6.
- [x] Rebuild KG từ replay event cho kết quả tương đương (P-M5-3) — `KnowledgeExtractor`/`sdk/kg_extract.py::extract_entities()` là HÀM THUẦN (không gọi LLM, không random, không đọc thời gian hệ thống — xem ADR-0028), phát lại CÙNG chuỗi event (`plan.created`/`script.created`/`summary.created`) theo CÙNG thứ tự luôn cho ra đồ thị TƯƠNG ĐƯƠNG. Kiểm bằng `tests/sdk/test_kg_extract.py::test_extract_entities_is_pure_and_reproducible` + toàn bộ pipeline chạy thật qua `build_daemon()` ở `tests/apps/paosd/test_knowledge_extractor_integration.py`.

**M5 — phạm vi lát cắt đã hoàn tất (P-M5-0 → P-M5-4), 2026-08-16.** 2/5 tiêu chí exit đạt TRỌN VẸN (Privacy Filter test đối kháng — P-M5-4; rebuild KG từ replay — P-M5-3). 3/5 tiêu chí còn lại đạt MỘT PHẦN vì lý do NẰM NGOÀI phạm vi một lát cắt code (không phải nợ kỹ thuật bị bỏ sót): (1) "không hỏi lại sở thích sau 3 job" và (2) "hành vi đổi ở job sau khi sửa tay" cần một Agent thật đọc lại Memory L3 đã học (BL-015) VÀ một UI thật để hỏi/không hỏi (M8) — cơ chế học/hạ confidence đã CHẠY THẬT, chỉ chưa có ai TIÊU THỤ kết quả; (3) "KG ≥ 100 node từ sử dụng thật" cần tích lũy dữ liệu dùng thật qua thời gian, không phải thứ một phiên code tạo ra trung thực được (P8). Không đóng milestone bằng cách hạ chuẩn 3 tiêu chí này xuống — chúng vẫn để `[ ]` mở, chờ đúng điều kiện thật đã ghi ở docs/backlog.md (BL-015, BL-019).
